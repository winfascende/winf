...
import { doc, getDoc, setDoc, collection, query, where, getDocs, addDoc, updateDoc, deleteDoc, limit } from 'firebase/firestore';
...
export const WinfProvider = ({ children }: { children?: ReactNode }) => {
  ...
  const completeJobAndGenerateWarranty = useCallback(async (jobId: string) => {
  ...
  const fetchLeads = useCallback(async () => {
    ...
    if (user?.id === 'proto-tiago-001') { 
      setLeads([
        { 
          id: 'lead1', user_id: 'proto-tiago-001', name: 'Ricardo Santos (Porsche 911)', contact: '11999991234', source: 'Instagram Ads', interest: 'NeoSkin PPF Gloss', status: 'Zona de Ataque', ai_score: 95, 
        ...
        { 
          id: 'lead2', user_id: 'proto-tiago-001', name: 'Amanda Carvalho (Audi Q8)', contact: '13988887766', source: 'W.A.R.P Search', interest: 'AeroCore NanoCeramic', status: 'Zona de Pressão', ai_score: 70,
        ...
        { 
          id: 'lead3', user_id: 'proto-tiago-001', name: 'Condomínio Prime (Arquitetura)', contact: '11977775544', source: 'Select Pro Portal', interest: 'Winf Select IR-99', status: 'Zona de Ataque', ai_score: 88,
  ...
  const updateLead = useCallback(async (id: string, updates: any) => {
    if (user?.id === 'proto-tiago-001') { setLeads(prev => prev.map(l => l.id === id ? { ...l, ...updates } : l)); return { success: true, error: null }; }
    try {
  ...
  const deleteLead = useCallback(async (id: string) => {
    if (user?.id === 'proto-tiago-001') { setLeads(prev => prev.filter(l => l.id !== id)); return { success: true, error: null }; }
    try {
  ...
  const fetchOrders = useCallback(async () => {
    if (!user?.id) return;
    if (user?.id === 'proto-tiago-001') { setOrders([]); return; } 
    try {
  ...
  const fetchInstallations = useCallback(async () => {
    if (!user?.id) return;
    if (user?.id === 'proto-tiago-001') { setInstallations([]); return; }
    try {
  ...
  const fetchSocialPosts = useCallback(async () => {
    if (!user?.id) return;
    if (user?.id === 'proto-tiago-001') { setSocialPosts([]); return; } 
    try {
  ...
  const fetchVehicles = useCallback(async () => {
    if (!user?.id) return;
    if (user?.id === 'proto-tiago-001') { setVehicles([]); return; } 
    try {
  ...
  const fetchMembers = useCallback(async () => {
    ...
    if (user?.id === 'proto-tiago-001') {
      setMembers([
        { id: 'u1', name: 'Tiago Winf', email: 'tiago@winf.com', role: 'Admin', winfCoins: 15000, w_rank_level: 'Sovereign' },
        { id: 'u2', name: 'João Silva', email: 'joao@parceiro.com', role: 'Licensee', winfCoins: 2500, w_rank_level: 'Master' },
        { id: 'u3', name: 'Maria Oliveira', email: 'maria@membro.com', role: 'Member', winfCoins: 500, w_rank_level: 'Initiate' },
        { id: 'u4', name: 'Carlos Souza', email: 'carlos@elite.com', role: 'Licensee', winfCoins: 8000, w_rank_level: 'Elite' },
      ]);
      return;
    }
    try {
  ...
  const fetchDocumentItems = useCallback(async () => {
    ...
    if (user?.id === 'proto-tiago-001') {
      setDocumentItems([
        {
          id: 'manual-financeiro-v5',
          title: 'Núcleo Estratégico & Unit Economics',
          category: 'Equity',
          content: `# MANUAL FINANCEIRO WINF™ PARTNERS V5.0
## UNIT ECONOMICS (ARQUITETURA)

A base de lucratividade do ecossistema Winf é fundamentada em margens agressivas sobre o custo do material.

### 1. WINF BLACKPRO® (Entry Level)
A porta de entrada. Ideal para volume e clientes focados em estética/privacidade.
- **Custo Rolo (45m²):** R$ 400,00
- **Custo m²:** R$ 8,88
- **Preço Venda m²:** R$ 120,00
- **Faturamento p/ Rolo:** R$ 5.400,00
- **LUCRO LÍQUIDO (Material): R$ 5.000,00 (Markup: 1.250%)**

### 2. WINF DUAL REFLECT® (Mid Tier)
O cavalo de batalha corporativo. Alta rejeição de calor com privacidade.
- **Custo Rolo (45m²):** R$ 800,00
- **Custo m²:** R$ 17,77
- **Preço Venda m²:** R$ 140,00
- **Faturamento p/ Rolo:** R$ 6.300,00
- **LUCRO LÍQUIDO (Material): R$ 5.500,00 (Markup: 687%)**

### 3. WINF INVISIBLE® (High Ticket)
A tecnologia cerâmica de elite. Transparência total e rejeição IR extrema.
- **Custo Rolo (45m²):** R$ 1.400,00
- **Custo m²:** R$ 31,11
- **Preço Venda m²:** R$ 260,00 (Médio)
- **Faturamento p/ Rolo:** R$ 11.700,00
- **LUCRO LÍQUIDO (Material): R$ 10.300,00 (Markup: 735%)**

## PROJEÇÃO DE ESCALA (CENÁRIO META)
Vendendo o mix: 3 Rolos BlackPro + 2 Rolos Dual + 1 Rolo Invisible/mês.
- **Investimento Estoque:** R$ 4.200,00
- **Faturamento Bruto:** R$ 40.500,00
- **Lucro Operacional:** R$ 36.300,00`,
          access_role: 'Member',
          created_at: new Date('2025-01-15').toISOString()
        },
        {
          id: 'j-licenciado',
          title: 'Blue Protocol: Jornada do Licenciado',
          category: 'Estratégia',
          content: `# BLUE PROTOCOL: A JORNADA DO ESPECIALISTA
## FASE 1: INICIAÇÃO (D+0)
- **Ativação Winf™ PARTNERS:** Configuração do ID Digital e carteira WinfCoin.
- **Treinamento Gênesis:** Domínio da retórica Winf vs Mercado Comum.
- **Kit de Branding:** Recebimento dos ativos visuais para redes sociais.

## FASE 2: OPERAÇÃO ALPHA (D+30)
- **NeuroMesh Integration:** Uso mandatório da IA para gerar visualizações para clientes.
- **Workflow de Garantia:** Todo serviço deve ser registrado para gerar XP no W-Rank.
- **Arsenal Tático:** Ativação de anúncios geolocalizados via Hub Central.

## FASE 3: ESCALA E DOMINÂNCIA (D+90)
- **Upgrade Territorial:** Transição de aplicador móvel para Ponto Físico ou Kiosk.
- **Squad Building:** Recrutamento de técnicos nível Operador para delegar a execução.`,
          access_role: 'Member',
          created_at: new Date('2025-02-10').toISOString()
        },
        {
          id: 'brand-bible-3',
          title: 'Brand Bible 3.0: O Tom da Soberania',
          category: 'Marca',
          content: `# WINF™ PARTNERS BRAND BIBLE 3.0
## POSICIONAMENTO: O PODER DO INVISÍVEL

A Winf não vende "películas". Vendemos **Tecnologia de Blindagem Molecular**.

### 1. TOM DE VOZ
- **Autoritário:** Não peça permissão para ser o melhor.
- **Técnico-Sofisticado:** Use termos como "Espectro Infravermelho", "Reflexão Seletiva" e "Pureza Ótica".
- **Frio:** Evite excesso de cores ou exclamações. O luxo é silencioso.

### 2. TERMINOLOGIA OBRIGATÓRIA
- **Célula de Blindagem** (em vez de película/insulfilm)
- **Jurisdição** (em vez de região/cidade)
- **Ativo Tecnológico** (em vez de produto)
- **AeroCore™** (em vez de cerâmica comum)`,
          access_role: 'Member',
          created_at: new Date('2025-03-05').toISOString()
        },
        {
          id: 'ops-kiosk',
          title: 'Operating Manual: Kiosk Retail Machine',
          category: 'Operacional',
          content: `# PROTOCOLO OPERACIONAL: WINF™ KIOSK
## OBJETIVO: FLUXO E CONVERSÃO

O Kiosk é uma antena de captação de tráfego de shopping para converter em tickets médios de R$ 450+.

### 1. ABORDAGEM ALPHA
Nunca pergunte "posso ajudar?". Use: "Já conhece a blindagem molecular para telas AeroCore?"

### 2. HARDWARE W-CUT
A calibração do plotter deve ser feita semanalmente. O uso de insumos não-homologados cancela a garantia da unidade.`,
          access_role: 'Licenciado',
          created_at: new Date('2025-03-20').toISOString()
        },
        { id: 'doc3', title: 'Ficha Técnica: WINF Dual Reflect', content: 'Película Arquitetônica Dual Reflect\nCategoria: Película de Controle Solar Arquitetônico\n...', category: 'Técnico', access_role: 'Licenciado', created_at: new Date().toISOString() }
      ]);
      return;
    }
    try {
  ...
  const markInsightAsRead = useCallback(async (id: string) => {
    if (!user?.id || user.id === 'proto-tiago-001') return { success: true, error: null };
    try {
  ...
  const contextValue: WinfContextType = {
    ...
    login, loginAsPrototype, logout, updateUserCoins, gamify, closeNotification,
